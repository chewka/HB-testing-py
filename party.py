...
def is_mel(name, email):
	...
	>>> is_mel('Melanie', 'mel@ubermelon.com')
	True


    """

    return name.strip().capitalize() == "Mel" or name.strip().capitalize() == "Mel Melitpolski" or email.strip().lower() == "mel@ubermelon.com"
...
def most_and_least_common_type(treats):
	...
	>>> treats = [{'type': 'dessert'},{'type': 'dessert'},{'type': 'drinks'},{'type': 'appetizer'},{'type': 'drink'},{'type': 'dessert'}]
	...
	>>> treats = [{'type' : 'dinosaur'}, {'type': 'broccoli'}]
	>>> most_and_least_common_type(treats)
	('dinosaur', 'dinosaur')
